...
//detail pagina
router.get("/:champId", auth, async function (req, res, next) {
	try {
...
router.delete("/:champId", auth, async (req, res, next) => {
	try {
		const doc = await Champ.findByIdAndDelete(req.params.champId)
...
router.patch("/:champId", auth, async (req, res) => {
	try {
		const doc = await Champ.findByIdAndUpdate(req.params.champId, req.body, {
			new: true,
			runValidators: true,
		})
...
// router.options("/:champId", (req, res) => {
// 	res.header("Access-Control-Allow-Methods", "PUT, POST, PATCH, DELETE, GET")
// 	res.header("Allow", "GET, PATCH, PUT, DELETE, OPTIONS")
// 	res.header("Access-Control-Allow-Origin", "*")
// 	res.header(
